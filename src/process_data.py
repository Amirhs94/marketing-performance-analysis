#!/usr/bin/env python3
# ETL + KPIs + Dashboard + Text Report
# Reads from ./data, writes to ./notebooks, script in ./src
...
2) Key Insights
   ...
   • Customer Acquisition Cost (CPA):
       ...
       - Note: Email campaigns have zero spend, so excluded from CPA comparison above.
   • Most effective at converting clicks into sales: {best_cvr_channel if best_cvr_channel else "—"} (CVR ≈ {fmt_pct(best_cvr_value)})
   • Trends: The weekly ROAS and CVR changes are mild; we can consider them as **steady-state** with no harsh fluctuations.
...
3) Charts & Dashboard
   ...
   • Combined dashboard.png created using all the above charts.
